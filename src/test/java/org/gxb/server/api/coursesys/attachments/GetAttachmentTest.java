...
public class GetAttachmentTest {
	...
	public void VerifyDataOk() {
		Response response = TestConfig.getOrDeleteExecu("get",
				"/attachments/" + attachmentid + "?access_token=" + access_token);

		response.then().assertThat().statusCode(200).body("status", equalTo(true)).body("data.id", equalTo(10));
	}
	...
	public void VerifyEmptyAccesstoken() {
		Response response = TestConfig.getOrDeleteExecu("get", "/attachments/" + attachmentid + "?access_token=");
	...
	public void VerifyEmptyAttachmentid() {
		Response response = TestConfig.getOrDeleteExecu("get", "/attachments/?access_token=" + access_token);
