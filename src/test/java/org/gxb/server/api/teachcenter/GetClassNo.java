 ...
 * http://w-api-f2.kaikeba.cn/tenants/8/classes?access_token=1aace0664359768b607e63b55cd8ed72&curPage=1&pageSize=1
 * 
 */
public class GetClassNo {
    ...
    public void test(){
		Response response= TestConfig.getOrDeleteExecu("get","/tenants/"+tenantid+"/classes?access_token="+token);
    ...
    public void testErrTenantId01(){
		Response response= TestConfig.getOrDeleteExecu("get","/tenants/"+999999999+"/classes?access_token="+token);
       response.then().
               assertThat().statusCode(401).
               body("status",equalTo(false)).body("message", equalTo("租户不存在"));
    }

	@Test(description="分页",priority=5)
    public void testPage(){
		Response response= TestConfig.getOrDeleteExecu("get","/tenants/"+9999999+"/classes?access_token="+token+"&curPage=2&pageSize=5");
       response.then().
               assertThat().statusCode(200).
               body("status",equalTo(true)).
