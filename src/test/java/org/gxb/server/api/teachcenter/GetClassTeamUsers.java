 ...
 * http://w-api-f2.kaikeba.cn/teams/8/user?access_token=1aace0664359768b607e63b55cd8ed72
 * 
 */
public class GetClassTeamUsers {
    ...
    public void testErrTeamid01(){
       ...
       response.then().
               ...
               body("status",equalTo(false)).body("message", equalTo("班组不存在"));
    }

	@Test(description="分页",priority=5)
    public void testPage(){
		Response response= TestConfig.getOrDeleteExecu("get","/teams/"+teamid+"/user?access_token="+token+"&curPage=2&pageSize=5");
       response.then().
               assertThat().statusCode(200).
               body("status",equalTo(true)).
