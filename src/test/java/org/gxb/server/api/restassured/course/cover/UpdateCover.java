 ...
 * http://192.168.30.33:8080/gxb-api/course/2/cover?loginUserId=123456
 */
public class UpdateCover {
    ...
    public void test() {
        ...
        jo.put("courseCover", "123.jpg");
        Response response = TestConfig.postOrPutExecu("put",
                "course/2/cover?loginUserId=123456", jo);

        response.then().log().all().assertThat()
                .statusCode(200)
                .body(equalTo("true"));
    }
    @Test(description = "课程封面不能为空", priority = 2)
    public void testWithEmptyCover() {
        JSONObject jo = new JSONObject();
        jo.put("courseCover", "");

        String response = HttpRequest.sendHttpPut(Url + "/course/-1/cover?loginUserId=123456", jo);
        response = response.substring(response.indexOf("&") + 1, response.length());
        JSONObject result = JSONObject.fromObject(response);

        Assert.assertEquals(result.get("code"),1000);
        Assert.assertEquals(result.get("type"),"ServiceException");
        Assert.assertEquals(result.get("message"),"课程封面不能为空");
    }
