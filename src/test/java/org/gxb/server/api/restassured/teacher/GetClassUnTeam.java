 ...
 *             http://w-api-f2.kaikeba.cn/classes/2083?access_token=
...
public class GetClassUnTeam {
	...
	public void test() {
		...
		response.then().
				...
				.body("data.id", equalTo(classid)).body("data.name", equalTo("响应式Web设计-单元得分"))
				.body("data.course.id", equalTo(1)).body("data.course.name", equalTo("响应式Web设计"))
	...
	public void testErrclassid01() {
		Response response = TestConfig.getOrDeleteExecu("get", "/classes/" + classid + "?access_token=" + token);
		response.then().assertThat().statusCode(401).body("status", equalTo(false)).body("message", equalTo("班次信息不存在"));
	}
