 ...
 * http://192.168.30.33:8080/gxb-api//course/2/introVideo?loginUserId=123456
 */
public class UpdateIntroVideo {
    ...
    public void test() {
        ...
        jo.put("introVideo", "123.jpg");
        Response response = TestConfig.postOrPutExecu("put",
                "/course/2/introVideo?loginUserId=123456", jo);

        response.then().log().all().assertThat()
                .statusCode(200)
                .body(equalTo("true"));
    }
    @Test(description = "课程介绍不能为空", priority = 2)
    public void testWithEmptyCover() {
        JSONObject jo = new JSONObject();
        jo.put("introVideo", "");

        String response = HttpRequest.sendHttpPut(Url + "/course/2/introVideo?loginUserId=123456", jo);
        response = response.substring(response.indexOf("&") + 1, response.length());
        JSONObject result = JSONObject.fromObject(response);

        Assert.assertEquals(result.get("code"),1000);
        Assert.assertEquals(result.get("type"),"ServiceException");
        Assert.assertEquals(result.get("message"),"课程介绍不能为空");
    }
    ...
    public void testWithCourseNotExist() {
        ...
        String response = HttpRequest.sendHttpPut(Url + "/course/-1/introVideo?loginUserId=123456", jo);
        response = response.substring(response.indexOf("&") + 1, response.length());
        JSONObject result = JSONObject.fromObject(response);
