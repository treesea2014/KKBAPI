...
public class SchoolFilter {
	...
	public void test() {
		...
		response.then().log().all().assertThat().statusCode(200).body("filter.tenantId", equalTo(tenantId))
	...
	public void testWithoutTenantId() {
		...
		response.then().log().all().assertThat().statusCode(200).body("dataList.name", Matchers.hasItem("辽宁科技大学"))
